...
const filename      = 'https://zsviczian.github.io/excalidraw/render-component.cljs';
const codeblockUID  = 'excalTEST';
...
async function createCodeBlock(blockUID,pageTitle, sourceCode) {
  let q= `[:find ?uid . :where [?e :node/title "${pageTitle}"][?e :block/uid ?uid]]`;
  let pageUID = window.roamAlphaAPI.q(q);
  if(pageUID == null) {
    pageUID = window.roamAlphaAPI.util.generateUID();
    await window.roamAlphaAPI.createPage({"page": 
                                          {"title": pageTitle, 
                                           "uid": pageUID}});
  }
  q = `[:find (max ?order) . :where [?p :block/uid "${pageUID}"][?p :block/children ?b][?b :block/order ?order]]`;
  const res = window.roamAlphaAPI.q(q);
  const order = (res == null) ? 0 : res + 1;
  console.log('page name: ', window.roamAlphaAPI.q(`[:find ?s . :where [?p :block/uid "${pageUID}"][?p :node/title ?s]]`));
...
const localVersion = getClojureNS(codeblockUID);
...
if (localVersion != latestVersion) {
  ...
  client.onreadystatechange = function() {
    if(client.readyState == 4) {
      ...
      const code = LZString.decompressFromUTF16(client.responseText);
      if (blockExists(codeblockUID))
        updateCodeBlock(codeblockUID,code);
      else
        createCodeBlock(codeblockUID,codePage,code);
    }
  }
  client.send();
}
